...
import { Download, Palette, Settings, Smartphone, Globe, FileText, User, Wifi, Gift, Image, Video, Link } from 'lucide-react';
...
export const QRGenerator: React.FC<QRGeneratorProps> = ({
  ...
  const downloadQR = async (format: 'png' | 'jpg' | 'svg') => {
    ...
    try {
      if (format === 'svg') {
        // For SVG, we'll extract the SVG element
  ...
  const isStandardFeature = (feature: string) => {
    return feature === 'logo' && tier === 'free';
  };
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (file && !isStandardFeature('logo')) {
                          const reader = new FileReader();
                          reader.onload = (e) => setLogo(e.target?.result as string);
                          reader.readAsDataURL(file);
                        }
                      }}
                      disabled={isStandardFeature('logo')}
                      className="mt-2"
                    />
                  </div>
                </TabsContent>
